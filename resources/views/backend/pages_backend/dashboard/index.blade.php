@extends('backend.layouts_backend.master')

@section('title')
Dashboard
@endsection
@section('content')

<!-- Main content dashboard  -->
<div class="page-content">
                    <div class="container-fluid">
                        <!-- start page title -->
                        <div class="row">
                            <div class="col-12">
                                <div class="page-title-box d-sm-flex align-items-center justify-content-between">
                                    <h4 class="mb-sm-0 font-size-18">Dashboard</h4>

                                    <div class="page-title-right">
                                        <ol class="breadcrumb m-0">
                                            <li class="breadcrumb-item"><a href="javascript: void(0);">Dashboard</a></li>
                                            <li class="breadcrumb-item active">Home</li>
                                        </ol>
                                    </div>

                                </div>
                            </div>
                        </div>
                        <!-- end page title -->

                        <!-- START OF DASHBOARD ROW 1 -->
                        <div class="row">
                            <div class="col-xl-3 col-md-6">
                                <!-- card -->
                                <div class="card card-h-100">
                                    <!-- card body -->
                                    <div class="card-body bg-gradient bg-success">
                                        <div class="row align-items-center">
                                            <div class="col-6">
                                            <span class="text-light mb-3 lh-1 d-block ">Photo Categories</span>                                                <h4 class="mb-3 text-light">
                                                    <span class="counter-value" data-target="value"></span>
                                                </h4>
                                            </div>
        
                                            
                                        </div>
                                        <div class="text-nowrap">
                                            <span class="badge bg-soft-primary text-success">valu</span>
                                            <span class="ms-1 text-muted font-size-13">Details</span>
                                        </div>
                                    </div><!-- end card body -->
                                </div><!-- end card -->
                            </div><!-- end col -->
        
                            <div class="col-xl-3 col-md-6">
                                <!-- card -->
                                <div class="card card-h-100">
                                    <!-- card body -->
                                    <div class="card-body bg-gradient bg-primary">
                                        <div class="row align-items-center">
                                            <div class="col-6">
                                                <span class="text-light mb-3 lh-1 d-block text-truncate">Photos</span>
                                                <h4 class="mb-3 text-light">
                                                    <span class="counter-value" data-target="value"></span>
                                                </h4>
                                            </div>
                                          
                                        </div>
                                        <div class="text-nowrap">
                                            <span class="badge bg-soft-danger text-light">value</span>
                                            <span class="ms-1 text-light font-size-13">Details</span>
                                        </div>
                                    </div><!-- end card body -->
                                </div><!-- end card -->
                            </div><!-- end col-->
        
                            <div class="col-xl-3 col-md-6">
                                <!-- card -->
                                <div class="card card-h-100">
                                    <!-- card body -->
                                    <div class="card-body bg-gradient bg-warning">
                                        <div class="row align-items-center">
                                            <div class="col-6">
                                                <span class="text-muted mb-3 lh-1 d-block text-truncate">News Articles</span>
                                                <h4 class="mb-3">
                                                    <span class="counter-value" data-target="value"></span>
                                                </h4>
                                            </div>
                                           
                                        </div>
                                        <div class="text-nowrap">
                                            <span class="badge bg-soft-success text-success">value</span>
                                            <span class="ms-1 text-muted font-size-13">Details</span>
                                        </div>
                                    </div><!-- end card body -->
                                </div><!-- end card -->
                            </div><!-- end col -->
        
                            <div class="col-xl-3 col-md-6">
                                <!-- card -->
                                <div class="card card-h-100">
                                    <!-- card body -->
                                    <div class="card-body bg-gradient bg-danger">
                                        <div class="row align-items-center">
                                            <div class="col-6">
                                                <span class="text-muted mb-3 lh-1 d-block text-truncate">Staff</span>
                                                <h4 class="mb-3">
                                                    <span class="counter-value" data-target="value"></span>
                                                </h4>
                                            </div>
                                           
                                        </div>
                                        <div class="text-nowrap">
                                            <span class="badge bg-soft-primary text-success">vlue</span>
                                            <span class="ms-1 text-light font-size-13">Details</span>
                                        </div>
                                    </div><!-- end card body -->
                                </div><!-- end card -->
                            </div><!-- end col -->    
                        </div>
                        <!-- END OF DASHBOARD ROW 1 -->
                        



                    </div>
                    <!-- container-fluid -->
                </div>

@endsection